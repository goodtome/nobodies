...
public class UploadRateLimitFilter extends OncePerRequestFilter {
    ...
    @Value("${upload.rate-limit.window-seconds}")
    ...
    protected boolean shouldNotFilter(HttpServletRequest request) {
        ...
        return !uri.startsWith("/videos/upload");
    }
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        if (record.counter.get() > maxRequests) {
            ...
            response.getWriter().write(objectMapper.writeValueAsString(ApiResponse.fail("Too many upload requests, please try again later.")));
            return;
        }
